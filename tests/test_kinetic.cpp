...
TEST_CASE("Kinetic energy matrix for H2O / STO-3G (regression test)")
{
    ...
    eri::math::DenseSquareMatrix T(n);

    for (std::size_t i = 0; i < n; ++i) {
        for (std::size_t j = 0; j <= i; ++j) {
            const double v =
                eri::one_electron::kinetic(basis[i], basis[j]);
    ...
    for (std::size_t i = 0; i < n; ++i) {
        for (std::size_t j = 0; j < n; ++j) {
            CHECK_MESSAGE(
                T(i,j) == doctest::Approx(ref[i][j]).epsilon(eps),
                "Mismatch at (" << i << "," << j << "): "
                << T(i,j) << " vs " << ref[i][j]
            );
        }
